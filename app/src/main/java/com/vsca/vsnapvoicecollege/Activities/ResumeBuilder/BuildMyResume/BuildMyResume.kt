    ...
    fun isGenerateResume() {
        ...
        if (fullData != null) {
            ...
            val contextJson = JsonObject().apply {
                // Basic Info
                addProperty("name", fullData?.memberName ?: "")
                addProperty("email", fullData?.memberstudentEmail ?: "")
                addProperty("phone", fullData?.memberPhoneNumber ?: "")
                addProperty("address", fullData?.memberPermanentAddress1 ?: "")
                ...
                add("internship", JsonArray().apply {
                    fullData?.internship?.forEach { intern ->
