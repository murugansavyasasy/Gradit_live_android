...
class Otp : AppCompatActivity() {
    ...
    private val otpEditTexts by lazy {
        listOf(
            binding.txtOtp1, binding.txtOtp2, binding.txtOtp3,
            binding.txtOtp4, binding.txtOtp5, binding.txtOtp6
        )
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.txtNumberlable.text =
            "Enter the verification code sent to +1(***) ***-$mobileNumber"

        binding.imgBack.setOnClickListener { onBackPressedDispatcher.onBackPressed() }

        binding.txtChangeNumber.setOnClickListener { finish() }

        setupOtpInputs()

        binding.btnVerifyContinue.setOnClickListener {
            output = otpEditTexts.joinToString("") { it.text.toString() }
            if (output.length == 6) {
                VerifiedOtp()
            } else {
                CommonUtil.ApiAlert(this, "Please enter the complete 6-digit code")
            }
        }
        ...
        appViewModel!!.VerifyOtp!!.observe(this) { response ->
            if (response != null) {
                if (response.Status == 1) {
                    Handler().postDelayed({
                        val intent = Intent(this@Otp, Create_Password::class.java)
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK)
                        startActivity(intent)
                    }, 1000)
                } else {
                    CommonUtil.ApiAlert(this, response.Message)
                }
            } else {
        ...
        otpEditTexts.forEachIndexed { index, editText ->
            editText.addTextChangedListener(object : TextWatcher {
                override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                    if (editText.text.toString().isNotEmpty() && index < otpEditTexts.lastIndex) {
