...
//            // Validation
            ...
            if (currentName != originalName ||currentPhone != originalPhone ||currentEmail != originalEmail ||currentPlacementStatus != originalPlacementStatus ||currentNotificationStatus != originalNotificationStatus ||CommonUtil.SelcetedFileList.isNotEmpty() ||isImageDeleted) {
                showConfirmationDialog()
            } else {
                Toast.makeText(this, "No changes found, saving anyway...", Toast.LENGTH_SHORT).show()
                isSaveTheData()
...
//        Log.d("MemberId2", memberId.toString())
        ...
        originalName = CommonUtil.saveBasicDetails?.memberName ?: ""
        originalPhone = CommonUtil.saveBasicDetails?.memberPhoneNumber ?: ""
        originalEmail = CommonUtil.saveBasicDetails?.memberstudentEmail ?: ""
        Log.d("EditoriginalEmail",originalEmail)
        originalPlacementStatus = CommonUtil.saveBasicDetails?.memberPlacementStatus ?: ""
        originalNotificationStatus = false

        memberId = CommonUtil.saveBasicDetails?.memberId?.toIntOrNull() ?: 0


        binding.edtName.setText(originalName)
        Log.d("EditBasicDetails", "Name: $originalName")
        binding.edtPhone.setText(originalPhone)
        Log.d("EditBasicDetails", "Phone: $originalPhone")
        binding.edtEmail.setText(originalEmail)
        Log.d("EditBasicDetails", "Email: $originalEmail")
        binding.edtPlacementStatus.setText(originalPlacementStatus)
        Log.d("EditBasicDetails", "PlacementStatus: $originalPlacementStatus")
        ...
        binding.customSwitch.setChecked(originalNotificationStatus)
    }
