...
class FilesViewActivity : AppCompatActivity()  {
    ...
    private lateinit var binding: FilePreviewActivityBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.lnrNext!!.setOnClickListener {
            if (currentPosition < isFilesList.size - 1) {
                currentPosition++
...
//        }
        ...
        for (i in CommonUtil.commonFileList.indices) {
            if (CommonUtil.commonFileList[i].path.contains("amazonaws.") || CommonUtil.commonFileList[i].path.contains(
                    "player.vimeo.com"
                ) || CommonUtil.commonFileList[i].type == CommonUtil.IMAGE || CommonUtil.commonFileList[i].type == CommonUtil.VIDEO
            ) {
                isFilesList.add(
                    CommonFileData(
                        type = CommonUtil.commonFileList[i].type,
                        path = CommonUtil.commonFileList[i].path
                    )
                )
            }
        }
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val response = RetrofitClient.apiService.getVideoDetails(
                    videoId,
                    "Bearer ${CommonUtil.isVimeoToken}"
                )
